...
test.describe('Delete Existing Todo Using Red X', () => {
  test('should be able to delete existing items on the page by clicking the red X next to them', async ({ page }) => {
    // Create a new todo locator
    const newTodo = page.getByPlaceholder('What needs to be done?');
    // Create 1st todo.
    await newTodo.fill(TODO_ITEMS[1]);
...
test.describe('Mark Todo Complete', () => {
  test('should be able to mark existing items on the page complete by clicking the circle next to them', async ({ page }) => {
    // Create a new todo locator
    const newTodo = page.getByPlaceholder('What needs to be done?');
    // Create 1st todo.
    await newTodo.fill(TODO_ITEMS[1]);
...
test.describe('Marked Todo Items DO NOT Show As Active', () => {
  test('should be able to only see uncomplete items in active view', async ({ page }) => {
    // Create a new todo locator
    const newTodo = page.getByPlaceholder('What needs to be done?');
    // Create 1st todo.
    await newTodo.fill(TODO_ITEMS[0]);
...
test.describe('Clear Complete Button', () => {
  test('should move completed todo items to Completed list', async ({ page }) => {
    // Create a new todo locator
    const newTodo = page.getByPlaceholder('What needs to be done?');
    // Create 1st todo.
    await newTodo.fill(TODO_ITEMS[0]);
